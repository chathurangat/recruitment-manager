package com.hms.oauth.provider;

import net.sf.json.JSONObject;
import net.sf.json.JSONSerializer;
import com.hms.oauth.api.impl.OAuth2Impl;
...
import com.hms.oauth.util.OAuthUtil;
...
import static com.hms.oauth.exception.OAuthErrorCode.*;
...
public class BitlyProvider extends OAuth2Impl{
    ...
    //bitly related keys for getting protected  resource
    ...
    public OAuthResponse getAccessToken(OAuthResponse requestTokenResponse) throws OAuthException{
        ...
        if(isTokenResponseValid(requestTokenResponse,REQUEST_TOKEN)){
            try{
                ...
                writer.close();
                os.close();
                LOGGER.info(" connecting with bitly.com .....");
                con.connect();
                ...
                LOGGER.debug(" MalformedURLException occurred with the ACCESS_TOKEN_ENDPOINT URL [{}] of the BiltyProvider class ", ACCESS_TOKEN_ENDPOINT);
                throw new OAuthException(" MalformedURLException occurred with BitlyProvider ACCESS_TOKEN_ENDPOINT URL ["+ACCESS_TOKEN_ENDPOINT+"] ",ex);
            }
            catch (UnsupportedEncodingException ex) {
                LOGGER.debug("URL Encoder [{}] is not supported ", URL_ENCODE);
                throw new OAuthException(String.format("URL Encode [%s] is not supported", URL_ENCODE),ex);
                ...
                LOGGER.debug(" IOException occurred in the BiltyProvider class and exception message [{}]", ex.getMessage());
                throw new OAuthException("IOException occurred in the BiltyProvider class",ex);
            }
        }
        return oAuthResponse;
    }
    ...
    public OAuthResponse getProtectedResource(OAuthResponse accessTokenResponse) throws OAuthException{
        ...
        if(isTokenResponseValid(accessTokenResponse,ACCESS_TOKEN)){
            try{
                ...
                LOGGER.debug(" MalformedURLException occurred with the ACCESS_TOKEN_ENDPOINT URL [{}] of the BiltyProvider class ", ACCESS_TOKEN_ENDPOINT);
                throw new OAuthException(" MalformedURLException occurred with BiltyProvider ACCESS_TOKEN_ENDPOINT URL ["+ACCESS_TOKEN_ENDPOINT+"] ",ex);
            }
            catch (UnsupportedEncodingException ex) {
                LOGGER.debug("URL Encoder [{}] is not supported ", URL_ENCODE);
                throw new OAuthException(String.format("URL Encode [%s] is not supported", URL_ENCODE),ex);
     ...
     * @throws com.hms.oauth.exception.OAuthException
     */
     ...
     *</p>
     * @param oAuthResponse as {@link com.hms.oauth.http.OAuthResponse}
    ...
    private Map<Object,Object> parseAccessTokenJsonResponseToMap(JSONObject jsonObject){
        ...
        Map<Object,Object> bitlyAccessTokenResponseMap =  new HashMap<Object, Object>();
        //extracting data from json object and populate them in Map
        bitlyAccessTokenResponseMap.put(ACCESS_TOKEN,jsonObject.get(ACCESS_TOKEN));
        bitlyAccessTokenResponseMap.put(TOKEN_TYPE,jsonObject.get(TOKEN_TYPE));
        bitlyAccessTokenResponseMap.put(EXPIRES_IN,jsonObject.get(EXPIRES_IN));
        bitlyAccessTokenResponseMap.put(ID_TOKEN,jsonObject.get(ID_TOKEN));
        return bitlyAccessTokenResponseMap;
    }
...
//        LOGGER.debug(" parsing JSON response [{}] and store response data in java.util.Map ", jsonObject);
        ...
        Map<Object,Object> bitlyAccessTokenResponseMap =  new HashMap<Object, Object>();
        //extracting data from json object and populate them in Map
        bitlyAccessTokenResponseMap.put(ID,jsonObject.get(ID));
        bitlyAccessTokenResponseMap.put(PICTURE,jsonObject.get(PICTURE));
        bitlyAccessTokenResponseMap.put(NAME,jsonObject.get(NAME));
        bitlyAccessTokenResponseMap.put(GIVEN_NAME,jsonObject.get(GIVEN_NAME));
        bitlyAccessTokenResponseMap.put(FAMILY_NAME,jsonObject.get(FAMILY_NAME));
        return bitlyAccessTokenResponseMap;
    }
